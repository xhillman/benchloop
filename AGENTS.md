...
- Whenever you complete a non-documentation task, provide a clear, concise summary of what you did, it's practical implications, any relevant context, and tradeoff decisions made in the chat. The developer should be able to understand the change and its impact without needing to look at the code. This helps maintain a clear history of changes and decisions for future reference. For example, you might say "Implemented the experiment creation endpoint in FastAPI, which allows users to create new experiments with a name and description. This is a critical part of the product workflow as it enables users to start organizing their AI experiments. I chose to keep the implementation straightforward without adding extra features like tagging or metadata for now, as those can be added later once we have the core functionality in place."
